 ...
 * Then, recording starts (in WAV formatting), and continues until the handset is replaced.
 * Playback button plasy last recorded message saved on SD card through earpiece 
 * 
 * Files are saved on SD card as 44.1kHz, 16-bit, mono signed integer RAW audio format 
 * This logic allows copying WAV files from the SD card via the USB connection
 * 
...
//    MTP.addFilesystem(SD, "SD Card");
    MTP.addFilesystem(SD, "TEENSY_SD"); // choose a nice name for the SD card volume to appear in your file explorer
    Serial.println("Added SD card via MTP");
    MTPcheckInterval = MTP.storage()->get_DeltaDeviceCheckTimeMS();
    
    // Value in dB
//  sgtl5000_1.micGain(15);
  sgtl5000_1.micGain(21); // much lower gain is required for the AOM5024 electret capsule
